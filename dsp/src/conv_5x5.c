...
STATUS_E dsp_fix_conv_5x5(FIX_MAP *p_input,	// pointer to input maps stored in flattened [maps][row][col] format.
	...
	o_h = (in_height + 2 * pad - 4 + stride - 1) / stride;
	o_w = (in_width  + 2 * pad - 4 + stride - 1) / stride;
	...
	if(use_dma) {
		dma_array(p_edma, p_weight + start_map * no_inputs * 25, p_conv_ker_buff[start_map % 2], no_inputs * 25 * sizeof(FIX_KER));
		wait_for_dma_tx(p_edma, FALSE, FALSE);
	} else {
		memcpy(p_conv_ker_buff[start_map % 2], p_weight + start_map * no_inputs * 25, no_inputs * 25 * sizeof(FIX_KER));
	...
	for(omap = start_map; omap < start_map + no_maps; omap++) {
		...
		if(omap < start_map + no_maps - 1) {
			if(use_dma) {
				dma_array(p_edma, p_weight + (omap + 1) * no_inputs * 25, p_conv_ker_buff[(omap + 1) % 2], no_inputs * 25 * sizeof(FIX_KER));
			} else {
				memcpy(p_conv_ker_buff[(omap + 1) % 2], p_weight + (omap + 1) * no_inputs * 25, no_inputs * 25 * sizeof(FIX_KER));
			}
		}
...
STATUS_E dsp_fix_conv_5x5_constrained(FIX_MAP *p_input,	// pointer to input maps stored in flattened [maps][row][col] format.
	...
	o_h = (in_height - 4 + stride - 1) / stride;
	o_w = (in_width  - 4 + stride - 1) / stride;

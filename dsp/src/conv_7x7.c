...
STATUS_E dsp_fix_conv_7x7(FIX_MAP *p_input,	// pointer to input maps stored in flattened [maps][row][col] format.
	...
	o_h = (in_height + 2 * pad - 6 + stride - 1) / stride;
	o_w = (in_width  + 2 * pad - 6 + stride - 1) / stride;
	...
	if(use_dma) {
		dma_array(p_edma, p_weight + start_map * no_inputs * 49, p_conv_ker_buff[start_map % 2], no_inputs * 49 * sizeof(FIX_KER));
		wait_for_dma_tx(p_edma, FALSE, FALSE);
	} else {
		memcpy(p_conv_ker_buff[start_map % 2], p_weight + start_map * no_inputs * 49, no_inputs * 49 * sizeof(FIX_KER));
	...
	for(omap = start_map; omap < start_map + no_maps; omap++) {
		...
		if(omap < start_map + no_maps - 1) {
			if(use_dma) {
				dma_array(p_edma, p_weight + (omap + 1) * no_inputs * 49, p_conv_ker_buff[(omap + 1) % 2], no_inputs * 49 * sizeof(FIX_KER));
			} else {
				memcpy(p_conv_ker_buff[(omap + 1) % 2], p_weight + (omap + 1) * no_inputs * 49, no_inputs * 49 * sizeof(FIX_KER));
			}
		}
		for(imap = 0; imap < no_inputs; imap++) {
			...
			for(out_row = 0; out_row < o_h; out_row++) {
				...
				IMG_conv_7x7_i16s_c16s(p_line_buff[0][0], // must be 64bit aligned
					p_temp_out_buff,	// must be 64bit aligned
					o_w_x8,				// must be x8
					pitch,				// must be x4
					p_conv_ker_buff[omap % 2] + imap * 49,
					shift
					);

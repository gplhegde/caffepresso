...
void filter2D(KERNEL_T *kernel, IMAGE_T *src, INTERMEDIATE_T *dest, unsigned kernel_width, unsigned src_width, unsigned conv_size, SCALE_T scale_fix){
	...
	for (row=0;row<conv_size;row++){
		...
		for (col=0;col<conv_size;col++){
			...
			sop += ((INTERMEDIATE_T)kernel[3])*((INTERMEDIATE_T)src[cntr[1]]);
			sop += ((INTERMEDIATE_T)kernel[4])*((INTERMEDIATE_T)src[cntr[1]+1]);
			sop += ((INTERMEDIATE_T)kernel[5])*((INTERMEDIATE_T)src[cntr[1]+2]);

			sop += ((INTERMEDIATE_T)kernel[6])*((INTERMEDIATE_T)src[cntr[2]]);
			sop += ((INTERMEDIATE_T)kernel[7])*((INTERMEDIATE_T)src[cntr[2]+1]);
			sop += ((INTERMEDIATE_T)kernel[8])*((INTERMEDIATE_T)src[cntr[2]+2]);

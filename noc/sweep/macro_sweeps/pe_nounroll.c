...
int main(void){
	...
	*mailboxes = 0xbeefdead;
	...
	num_maps = (GLOBAL_CONSTANTS_T *)0xGLOBAL_CONSTANTS_ADDR;
...
#ifdef MID_PRECISION
	float *image_float = (float *)scratchpad;
	floatify(image,image_float,IMAGE_SIZE);
	scratchpad += IMAGE_SIZE;
	PTR_UNROLL(MAP_UNROLL_FACTOR,FLOAT_KERNEL_INIT,0);
	scratchpad += KERNEL_SIZE*MAP_UNROLL_FACTOR;
#endif

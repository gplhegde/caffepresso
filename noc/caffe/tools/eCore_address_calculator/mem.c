...
#ifndef DEFINES_H
#define DEFINES_H
#define L1_MAPS_PER_ECORE 4
#define L2_MAPS_PER_ECORE 8
...
int main (int argc, char **argv) {
	...
	//2 global constants per eCore -- L1_num_maps & L2_num_maps
	unsigned globals = e_malloc(2*sizeof(GLOBAL_CONSTANTS_T),sizeof(GLOBAL_CONSTANTS_T)); 
	...
	unsigned l1map_addr = e_malloc(L2_MAPS_PER_ECORE*L1_TO_L2*sizeof(MEM_ADDR_T),sizeof(MEM_ADDR_T));
	unsigned l1map_state = e_malloc(L2_MAP_UNROLL_FACTOR*UNROLL_FACTOR*L1_MAP_SIZE*sizeof(L1_MAP_T),sizeof(L1_MAP_T));
	unsigned mailbox = e_malloc(4*sizeof(unsigned),sizeof(unsigned));

	//let's work out scratchpad usage
	//inside L1
	unsigned scratch_s = 0;
	unsigned conv_size = IMG_WIDTH - L1_KERNEL_WIDTH +1;
	...
	unsigned l1_image_floats = IMG_SIZE*sizeof(float);
...
//	printf("L1 scratchpad size = %d\n",scratch_s);
	...
	temp += NUM_BUFFER*conv_size*conv_size*sizeof(INTERMEDIATE_T);//number of filter2D_out buffers
	temp += conv_size*conv_size*sizeof(INTERMEDIATE_T);//accum vector
	//don't have to transfer scale values anymore..
	//temp += UNROLL_FACTOR*sizeof(MEMCPY_CHUNK_T)*(sizeof(SCALE_T)/sizeof(MEMCPY_CHUNK_T));
#ifdef MID_PRECISION
	unsigned l2_kernel_floats = NUM_BUFFER*L2_KERNEL_SIZE*sizeof(float);
	unsigned l2_map_floats = NUM_BUFFER*L1_MAP_SIZE*sizeof(float);
	temp += l2_kernel_floats + l2_map_floats;
#endif

...
//	__asm__ __volatile__("ldr %0, [%1]":"=r"(time_s):"r"(ctimer):);
	...
	for (i=0;i<*L2_num_maps;i++){
		...
		filter2D(&L2_kernel[i],(l1map_state+L1_MAP_SIZE),dummy,filter2D_out1,L2_KERNEL_WIDTH,in_size,L2_kernel_scale[i],0);
		
		e_dma_copy(dest2,src2,L1_MAP_SIZE*sizeof(uint16_t));
		filter2D(&L2_kernel[i],(l1map_state+2*L1_MAP_SIZE),dummy,filter2D_out2,L2_KERNEL_WIDTH,in_size,L2_kernel_scale[i],0);
		
		e_dma_copy(dest3,src3,L1_MAP_SIZE*sizeof(uint16_t));
		filter2D(&L2_kernel[i],(l1map_state+3*L1_MAP_SIZE),dummy,filter2D_out3,L2_KERNEL_WIDTH,in_size,L2_kernel_scale[i],0);
		
		e_dma_copy(dest4,src4,L1_MAP_SIZE*sizeof(uint16_t));
		filter2D(&L2_kernel[i],(l1map_state+4*L1_MAP_SIZE),dummy,filter2D_out4,L2_KERNEL_WIDTH,in_size,L2_kernel_scale[i],0);
		
		e_dma_copy(dest5,src5,L1_MAP_SIZE*sizeof(uint16_t));
		filter2D(&L2_kernel[i],(l1map_state+5*L1_MAP_SIZE),dummy,filter2D_out5,L2_KERNEL_WIDTH,in_size,L2_kernel_scale[i],0);
		
		e_dma_copy(dest6,src6,L1_MAP_SIZE*sizeof(uint16_t));
		filter2D(&L2_kernel[i],(l1map_state+6*L1_MAP_SIZE),dummy,filter2D_out6,L2_KERNEL_WIDTH,in_size,L2_kernel_scale[i],0);
		
		e_dma_copy(dest7,src7,L1_MAP_SIZE*sizeof(uint16_t));
		filter2D(&L2_kernel[i],(l1map_state+7*L1_MAP_SIZE),dummy,filter2D_out7,L2_KERNEL_WIDTH,in_size,L2_kernel_scale[i],0);
		
		e_dma_copy(dest8,src8,L1_MAP_SIZE*sizeof(uint16_t));
		filter2D(&L2_kernel[i],(l1map_state+8*L1_MAP_SIZE),dummy,filter2D_out8,L2_KERNEL_WIDTH,in_size,L2_kernel_scale[i],0);
		
		e_dma_copy(dest9,src9,L1_MAP_SIZE*sizeof(uint16_t));
		filter2D(&L2_kernel[i],(l1map_state+9*L1_MAP_SIZE),dummy,filter2D_out9,L2_KERNEL_WIDTH,in_size,L2_kernel_scale[i],0);
		
		for (k=0;k<conv_size*conv_size;k++){
			filter2D_out1[k] += filter2D_out2[k];
			filter2D_out3[k] += filter2D_out4[k];
			filter2D_out5[k] += filter2D_out6[k];
			filter2D_out7[k] += filter2D_out8[k];
			filter2D_out9[k] += filter2D_out[k];

			filter2D_out1[k] += filter2D_out3[k];
			filter2D_out5[k] += filter2D_out7[k];
			accum[k] += filter2D_out9[k];

			filter2D_out1[k] += filter2D_out5[k];
			accum[k] += filter2D_out1[k];
		}

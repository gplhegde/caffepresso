...
//	L1_L2_edge_kernel = (unsigned **)malloc(L2_MAPS*sizeof(unsigned *));
	...
	for (i=0;i<L2_MAPS;i++){	
		...
		//L1_L2_edge_kernel[i] = (unsigned *)malloc(L1_TO_L2*sizeof(unsigned));
	}
...
//			L1_L2_edge_kernel[m][i] = rand()%L2_MAPS;
		}
	}
}
...
//	l2kernel_addr_shard = (unsigned **)malloc(NUM_PES*sizeof(unsigned *));
//	l2kernel_scale_addr_shard = (unsigned **)malloc(NUM_PES*sizeof(unsigned *));
	for (i=0;i<NUM_PES;i++){
...
//		l2kernel_addr_shard[i] = (unsigned *)malloc(L1_TO_L2*L2_MAX_MAPS*sizeof(unsigned));
//		l2kernel_scale_addr_shard[i] = (unsigned *)malloc(L1_TO_L2*L2_MAX_MAPS*sizeof(unsigned));
	}
...
//	unsigned *l2kernel_addr = (unsigned *)malloc(L2_MAPS*sizeof(unsigned));
//	unsigned *l2kernel_scale_addr = (unsigned *)malloc(L2_MAPS*sizeof(unsigned));
	...
	for (i=0;i<L2_MAPS;i++){
		...
		while (!success){
			if (l2map_c[pe] < L2_MAX_MAPS){
				...
				//l2kernel_addr[i] = l2kernel_global_addr(pe,l2map_c[pe]);
				//l2kernel_scale_addr[i] = l2kernel_scale_global_addr(pe,l2map_c[pe]);
				l2map_c[pe]++;
				success = 1;
			} else {
				pe = (pe+1)%NUM_PES;
		...
		for (j=0;j<L2_MAPS;j++){
			if (placement_L2[j] == i){
				for (k=0;k<L1_TO_L2;k++){
					...
					//l2kernel_addr_shard[i][index] = l2kernel_addr[L1_L2_edge_kernel[j][k]];
					//l2kernel_scale_addr_shard[i][index] = l2kernel_scale_addr[L1_L2_edge_kernel[j][k]];
					index++;
				}
			}
		}

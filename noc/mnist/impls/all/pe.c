...
//	l2kernel_addr = (unsigned *)L2_L2_KERNEL_ADDR;
//	l2kernel_state = (uint8_t *)L2_L2_KERNEL_STATE_ADDR;
//	l2kernel_scale_addr = (unsigned *)L2_L2_KERNEL_SCALE_ADDR;
//	l2kernel_scale_state = (float *)L2_L2_KERNEL_SCALE_STATE_ADDR;
	...
	float *accum = (float *)(filter2D_out4 + conv_size*conv_size);
	uint16_t *temp = (uint16_t *)(accum + conv_size*conv_size);//for storing fractured results
	...
	for (i=0;i<*L2_num_maps;i++){
		...
		for (j=0;j<L1_TO_L2;j+=UNROLL_FACTOR){
			...
			//uint8_t *k_src = (uint8_t *)(l2kernel_addr[i*L1_TO_L2+j]);
			//uint8_t *k_dest = (uint8_t *)(l2kernel_state);
			//unsigned *s_src = (unsigned *)(l2kernel_scale_addr[i*L1_TO_L2+j]);
			//float *s_dest = (float *)(l2kernel_scale_state);
			
			uint16_t *src1 = (uint16_t*)(l1map_addr[i*L1_TO_L2+j+1]);
			uint16_t *dest1 = (uint16_t *)(l1map_state + L1_MAP_SIZE);
			//uint8_t *k_src1 = (uint8_t *)(l2kernel_addr[i*L1_TO_L2+j+1]);
			//uint8_t *k_dest1 = (uint8_t *)(l2kernel_state + L2_KERNEL_SIZE);
			//unsigned *s_src1 = (unsigned *)(l2kernel_scale_addr[i*L1_TO_L2+j+1]);
			//float *s_dest1 = (float *)(l2kernel_scale_state + 1);
			
			uint16_t *src2 = (uint16_t *)(l1map_addr[i*L1_TO_L2+j+2]);
			uint16_t *dest2 = (uint16_t *)(l1map_state + 2*L1_MAP_SIZE); 
			//uint8_t *k_src2 = (uint8_t *)(l2kernel_addr[i*L1_TO_L2+j+2]);
			//uint8_t *k_dest2 = (uint8_t *)(l2kernel_state + 2*L2_KERNEL_SIZE);
			
			uint16_t *src3 = (uint16_t *)(l1map_addr[i*L1_TO_L2+j+3]);
			uint16_t *dest3 = (uint16_t *)(l1map_state + 3*L1_MAP_SIZE); 
			//uint8_t *k_src3 = (uint8_t *)(l2kernel_addr[i*L1_TO_L2+j+3]);
			//uint8_t *k_dest3 = (uint8_t *)(l2kernel_state + 3*L2_KERNEL_SIZE);
			
			uint16_t *src4 = (uint16_t *)(l1map_addr[i*L1_TO_L2+j+4]);
			uint16_t *dest4 = (uint16_t *)(l1map_state + 4*L1_MAP_SIZE); 
			//uint8_t *k_src4 = (uint8_t *)(l2kernel_addr[i*L1_TO_L2+j+4]);
			//uint8_t *k_dest4 = (uint8_t *)(l2kernel_state + 4*L2_KERNEL_SIZE);
			
			e_dma_copy(dest,src,L1_MAP_SIZE*sizeof(uint16_t));
			//e_dma_copy(k_dest,k_src,L2_KERNEL_SIZE*sizeof(uint8_t));
			//e_dma_copy(s_dest,s_src,sizeof(float));
			e_dma_copy(dest1,src1,L1_MAP_SIZE*sizeof(uint16_t));
			//e_dma_copy(k_dest1,k_src1,L2_KERNEL_SIZE*sizeof(uint8_t));
			//e_dma_copy(s_dest1,s_src1,sizeof(float));
			e_dma_copy(dest2,src2,L1_MAP_SIZE*sizeof(uint16_t));
			//e_dma_copy(k_dest2,k_src2,L2_KERNEL_SIZE*sizeof(uint8_t));
			//e_dma_copy(s_dest2,s_src2,sizeof(float));
			e_dma_copy(dest3,src3,L1_MAP_SIZE*sizeof(uint16_t));
			//e_dma_copy(k_dest3,k_src3,L2_KERNEL_SIZE*sizeof(uint8_t));
			//e_dma_copy(s_dest3,s_src3,sizeof(float));
			e_dma_copy(dest4,src4,L1_MAP_SIZE*sizeof(uint16_t));
			//e_dma_copy(k_dest4,k_src4,L2_KERNEL_SIZE*sizeof(uint8_t));
			//e_dma_copy(s_dest4,s_src4,sizeof(float));

			/*
			uint16_t *s_src = (uint16_t *)(l2kernel_scale_addr[i*L1_TO_L2+j]);
			float *s_dest = (float *)(l2kernel_scale_state);
			uint16_t *s_src1 = (uint16_t *)(l2kernel_scale_addr[i*L1_TO_L2+j+1]);
			float *s_dest1 = (float *)(l2kernel_scale_state + 1);
			uint16_t *s_src2 = (uint16_t *)(l2kernel_scale_addr[i*L1_TO_L2+j+2]);
			float *s_dest2 = (float *)(l2kernel_scale_state + 2);
			uint16_t *s_src3 = (uint16_t *)(l2kernel_scale_addr[i*L1_TO_L2+j+3]);
			float *s_dest3 = (float *)(l2kernel_scale_state + 3);
			uint16_t *s_src4 = (uint16_t *)(l2kernel_scale_addr[i*L1_TO_L2+j+4]);
			float *s_dest4 = (float *)(l2kernel_scale_state + 4);

			//fracture into 2-byte chunks memcpy optimization
			memcpy(temp,s_src,sizeof(uint16_t));
			memcpy((temp+1),(s_src+1),sizeof(uint16_t));
			memcpy((temp+2),s_src1,sizeof(uint16_t));
			memcpy((temp+3),(s_src1+1),sizeof(uint16_t));
			memcpy((temp+4),s_src2,sizeof(uint16_t));
			memcpy((temp+5),(s_src2+1),sizeof(uint16_t));
			memcpy((temp+6),s_src3,sizeof(uint16_t));
			memcpy((temp+7),(s_src3+1),sizeof(uint16_t));
			memcpy((temp+8),s_src4,sizeof(uint16_t));
			memcpy((temp+9),(s_src4+1),sizeof(uint16_t));

			float *read = (float *)(temp);
			float *read1 = (float *)(temp+2);
			float *read2 = (float *)(temp+4);
			float *read3 = (float *)(temp+6);
			float *read4 = (float *)(temp+8);
			*s_dest = *read;
			*s_dest1 = *read1;
			*s_dest2 = *read2;
			*s_dest3 = *read3;
			...
			*/
			...
			filter2D(&L2_kernel[i],(l1map_state+L1_MAP_SIZE),k_dest,filter2D_out1,L2_KERNEL_WIDTH,in_size,L2_kernel_scale[i],0);
			filter2D(&L2_kernel[i],(l1map_state+2*L1_MAP_SIZE),k_dest,filter2D_out2,L2_KERNEL_WIDTH,in_size,L2_kernel_scale[i],0);
			filter2D(&L2_kernel[i],(l1map_state+3*L1_MAP_SIZE),k_dest,filter2D_out3,L2_KERNEL_WIDTH,in_size,L2_kernel_scale[i],0);
			filter2D(&L2_kernel[i],(l1map_state+4*L1_MAP_SIZE),k_dest,filter2D_out4,L2_KERNEL_WIDTH,in_size,L2_kernel_scale[i],0);
			

			for (k=0;k<conv_size*conv_size;k++){
				filter2D_out[k] += filter2D_out1[k];
				filter2D_out2[k] += filter2D_out3[k];
				accum[k] += filter2D_out4[k];
				accum[k] += filter2D_out[k] + filter2D_out2[k];
			}
		}
...
/*
			...
			for (kernel_row=0;kernel_row<kernel_width;kernel_row++){
				if (isL1) {//image input, 7*7 kernel
					sop += ((float)kernel[kernel_row*kernel_width])*((float)src1[cntr[kernel_row]]);
					sop += ((float)kernel[kernel_row*kernel_width+1])*((float)src1[cntr[kernel_row]+1]);
					sop += ((float)kernel[kernel_row*kernel_width+2])*((float)src1[cntr[kernel_row]+2]);
					sop += ((float)kernel[kernel_row*kernel_width+3])*((float)src1[cntr[kernel_row]+3]);
					sop += ((float)kernel[kernel_row*kernel_width+4])*((float)src1[cntr[kernel_row]+4]);
					sop += ((float)kernel[kernel_row*kernel_width+5])*((float)src1[cntr[kernel_row]+5]);
					sop += ((float)kernel[kernel_row*kernel_width+6])*((float)src1[cntr[kernel_row]+6]);
				} else {//L1 map input, 5*5 kernel
					sop += ((float)kernel[kernel_row*kernel_width])*((float)src[cntr[kernel_row]]);
					sop += ((float)kernel[kernel_row*kernel_width+1])*((float)src[cntr[kernel_row]+1]);
					sop += ((float)kernel[kernel_row*kernel_width+2])*((float)src[cntr[kernel_row]+2]);
					sop += ((float)kernel[kernel_row*kernel_width+3])*((float)src[cntr[kernel_row]+3]);
					sop += ((float)kernel[kernel_row*kernel_width+4])*((float)src[cntr[kernel_row]+4]);
				}
			}
*/
/*

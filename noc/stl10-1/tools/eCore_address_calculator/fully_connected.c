...
int main (int argc, char **argv) {
	...
	unsigned patch_width_from_image = patch_widths[0]*subsampling_factors[0] + kernel_widths[0] - 1;
	unsigned patch_height_from_image = patch_heights[0]*subsampling_factors[0] + kernel_widths[0] - 1;
	...
	for (i=0;i<NUM_LAYERS;i++){
		if (i==0){
				num_patches[i] = ((IMAGE_WIDTH - kernel_widths[i] + 1)/subsampling_factors[i])/patch_widths[i];
				num_patches[i] = num_patches[i]*((IMAGE_HEIGHT - kernel_widths[i] + 1)/subsampling_factors[i])/patch_heights[i];

void patch(unsigned in_patch_height, unsigned in_patch_width, unsigned *out_patch_height, unsigned *out_patch_width, 
	unsigned num_layers, unsigned out_layer_height, unsigned out_layer_width, unsigned subsampling_factors, 
	unsigned kernel_dims) {
...
//	printf("Patch dim final = %dx%d\n",curr_patch_dim,curr_patch_dim);
	for (i=num_layers;i>0;i--){
		...
		curr_patch_height = curr_patch_height*subsampling_factors;
		curr_patch_width = curr_patch_width*subsampling_factors;
		curr_patch_height = curr_patch_height + kernel_dims - 1;
		curr_patch_width = curr_patch_width + kernel_dims - 1;
